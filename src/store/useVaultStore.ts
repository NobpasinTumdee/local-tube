...
/** Everything persisted. `digests` and `digestSalt` are deliberately plaintext — see cryptoUtils. */
interface VaultRecord {
  payload: EncryptedPayload;
  digestSalt: Uint8Array;
  digests: string[];
  createdAt: number;
}
...
interface VaultState {
  hydrated: boolean;
  /** A vault has been set up (independent of whether it is open). */
  hasVault: boolean;
  isVaultUnlocked: boolean;
  /** Decrypted membership. Empty whenever locked. */
  mediaIds: string[];
  /** Salted hashes of the vaulted ids — present even while locked, to hide them. */
  hiddenDigests: string[];
  digestSalt: Uint8Array | null;
  busy: boolean;
  error: string | null;

  hydrate: () => Promise<void>;
  createVault: (pin: string) => Promise<boolean>;
  unlock: (pin: string) => Promise<boolean>;
  lock: () => void;
  touch: () => void;
  addToVault: (mediaId: string) => Promise<void>;
  removeFromVault: (mediaId: string) => Promise<void>;
  destroyVault: (pin: string) => Promise<boolean>;
  setError: (msg: string | null) => void;
}

export const useVaultStore = create<VaultState>()((set, get) => {
  /** Persists the current membership under the live session key. */
  async function persistMembership(ids: string[]): Promise<void> {
    if (!sessionKey || !sessionSalt) throw new Error('Vault is locked.');
    const existing = await idbGet<VaultRecord>(VAULT_KEY);
  ...
  return {
    ...
    hydrate: async () => {
      ...
      try {
        const rec = await idbGet<VaultRecord>(VAULT_KEY);
        set({
          hydrated: true,
          hasVault: !!rec,
    ...
    unlock: async (pin) => {
      ...
      try {
        ...
        const ids = await decryptWithKey<string[]>(rec.payload, key);
    ...
    destroyVault: async (pin) => {
      ...
      try {
        ...
        if (rec) {
          ...
          await decryptWithKey<string[]>(rec.payload, key); // throws on a wrong PIN
        }
