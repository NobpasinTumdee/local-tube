...
/** Result of loading a preset, so the UI can explain a partial restore. */
export interface LoadPresetResult {
  loaded: number;
  /** Folder names the user declined, or that no longer exist. */
  denied: string[];
}

interface LibraryState {
  /** Mounted folders. Every entry is permission-verified and readable. */
  activeHandles: FileSystemDirectoryHandle[];
  presets: WorkspacePreset[];

  /**
   * Restored from IDB but not yet usable — the grant lapsed with the browser
   * session. Needs a click to re-prompt; see restorePending().
   */
  pendingRestore: FileSystemDirectoryHandle[];

  /** False until the first IDB read finishes, so the UI can avoid flashing. */
  hydrated: boolean;
  busy: boolean;
  error: string | null;

  hydrate: () => Promise<void>;
  addHandleToActive: (handle: FileSystemDirectoryHandle) => Promise<void>;
  removeHandleFromActive: (handle: FileSystemDirectoryHandle) => Promise<void>;
  clearActiveHandles: () => Promise<void>;
  restorePending: () => Promise<void>;
  dismissPending: () => Promise<void>;
  saveActiveAsPreset: (presetName: string) => Promise<void>;
  loadPreset: (presetId: string) => Promise<LoadPresetResult>;
 ...
 * no path to compare.
 */
async function indexOfHandle(
  list: readonly FileSystemDirectoryHandle[],
  handle: FileSystemDirectoryHandle,
): Promise<number> {
  for (let i = 0; i < list.length; i++) {
    try {
      if (await list[i].isSameEntry(handle)) return i;
    } catch {
...
export const useLibraryStore = create<LibraryState>()((set, get) => ({
  ...
  hydrate: async () => {
    ...
    try {
      const [storedActive, storedPresets] = await Promise.all([
        idbGet<FileSystemDirectoryHandle[]>(ACTIVE_KEY),
        idbGet<WorkspacePreset[]>(PRESETS_KEY),
      ]);
      ...
      const readable: FileSystemDirectoryHandle[] = [];
      for (const h of granted) {
        if (await isDirectoryReadable(h)) readable.push(h);
      }
  ...
  restorePending: async () => {
    ...
    const stillPending: FileSystemDirectoryHandle[] = [];
    const restored: FileSystemDirectoryHandle[] = [];
    for (const h of pending) {
      if (await verifyPermission(h, 'read') && await isDirectoryReadable(h)) restored.push(h);
    ...
    for (const h of restored) {
      if (await indexOfHandle(merged, h) < 0) merged.push(h);
    }
  ...
  loadPreset: async (presetId) => {
    ...
    if (!preset) return { loaded: 0, denied: [] };

    set({ busy: true, error: null });
    try {
      const usable: FileSystemDirectoryHandle[] = [];
      const denied: string[] = [];
      ...
      for (const h of preset.handles) {
        ...
        if (await verifyPermission(h, 'read') && await isDirectoryReadable(h)) {
          if (await indexOfHandle(usable, h) < 0) usable.push(h);
        } else {
          denied.push(h.name);
        }
      }
