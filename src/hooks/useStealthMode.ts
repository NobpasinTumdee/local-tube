 ...
 */
const priorMuted = new WeakMap<HTMLMediaElement, boolean>();

/** Every document the app renders into — the main one, plus any PiP window. */
function mediaDocuments(): Document[] {
  const docs = [document];
  const pip = getPiPWindow();
  if (pip && !pip.closed) docs.push(pip.document);
  return docs;
}
...
function allMediaElements(): HTMLMediaElement[] {
  ...
  for (const doc of mediaDocuments()) {
    out.push(...Array.from(doc.querySelectorAll<HTMLMediaElement>('video, audio')));
  }
  return out;
}

function silence(el: HTMLMediaElement) {
  if (!priorMuted.has(el)) priorMuted.set(el, el.muted);
  el.muted = true;
}

function restore(el: HTMLMediaElement) {
...
export function muteAllMedia(): void {
  allMediaElements().forEach(silence);
}

export function restoreAllMedia(): void {
 ...
 */
export function useStealthMode(): void {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (!matchesCombo(e, shortcut)) return;
      /*
      ...
      const mo = new MutationObserver((records) => {
        for (const rec of records) {
          for (const node of Array.from(rec.addedNodes)) {
            ...
            node.querySelectorAll?.<HTMLMediaElement>('video, audio').forEach(silence);
          }
        }
      });
