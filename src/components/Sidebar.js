...
function NavItem({ icon, label, count, active, onClick, }) {
    return (_jsxs("button", { onClick: onClick, className: `group relative mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all duration-200 ${active
            ...
            : 'text-content/65 hover:bg-content/[0.04] hover:text-content'}`, children: [active && _jsx(ActiveBar, {}), _jsx("span", { className: `shrink-0 transition-transform duration-200 ${active ? 'text-primary' : 'group-hover:scale-110'}`, children: icon }), _jsx("span", { className: "flex-1 truncate text-left", children: label }), count != null && (_jsx("span", { className: "text-[11px] tabular-nums text-content/30", children: count }))] }));
}
