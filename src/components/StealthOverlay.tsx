...
export default function StealthOverlay() {
  ...
  return createPortal(
    <div
      ...
      className={`fixed inset-0 z-[9999] select-none outline-none backdrop-blur-3xl ${
        style === 'terminal' ? 'bg-[#08090b]' : 'bg-black'
      }`}
    >
      {style === 'terminal' ? <FakeTerminal /> : null}

      <p
        className={`pointer-events-none absolute bottom-4 right-5 font-mono text-[11px] tracking-wide transition-opacity duration-700 ${
          showHint ? 'opacity-25' : 'opacity-0'
        } ${style === 'terminal' ? 'text-emerald-400' : 'text-white'}`}
      >
...
function FakeTerminal() {
  ...
  return (
    <div className="h-full w-full overflow-hidden p-6 font-mono text-[13px] leading-relaxed text-emerald-400/85 sm:p-10">
      ...
      {visible >= LOG_LINES.length && (
        <span className="ml-[1ch] inline-block h-[1.1em] w-[0.6ch] translate-y-[0.2em] animate-pulse bg-emerald-400/85" />
      )}
    </div>
  );
}
