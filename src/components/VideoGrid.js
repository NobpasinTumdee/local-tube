...
function ViewToggleBtn({ label, icon, value, current, onClick, }) {
    ...
    return (_jsxs("button", { onClick: () => onClick(value), className: `flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[11px] font-semibold transition ${active
            ...
            : 'text-content/40 hover:bg-content/5 hover:text-content/70'}`, title: label, children: [icon, label] }));
}
...
function FolderCard({ folder, onClick }) {
    return (_jsxs("button", { onClick: onClick, className: "group flex items-center gap-3 rounded-xl border border-content/5 bg-content/[0.03] px-4 py-3.5 text-left transition hover:border-content/10 hover:bg-content/[0.07]", children: [_jsx("div", { className: "flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-yellow-400/10", children: _jsx("svg", { xmlns: "http://www.w3.org/2000/svg", className: "h-5 w-5 text-yellow-400/80", viewBox: "0 0 24 24", fill: "currentColor", children: _jsx("path", { d: "M20 6h-8l-2-2H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2z" }) }) }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("p", { className: "truncate text-sm font-medium text-content/85 group-hover:text-content", children: folder.name }), _jsxs("p", { className: "text-xs text-content/35", children: [folder.mediaCount, " item", folder.mediaCount !== 1 ? 's' : '', folder.children.length > 0 && ` · ${folder.children.length} folder${folder.children.length !== 1 ? 's' : ''}`] })] }), _jsx("svg", { xmlns: "http://www.w3.org/2000/svg", className: "h-4 w-4 shrink-0 text-content/20 transition group-hover:text-content/50", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", children: _jsx("polyline", { points: "9 18 15 12 9 6" }) })] }));
}
