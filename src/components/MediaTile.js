...
export default function MediaTile({ slot, mediaId, onRegister }) {
    ...
    useEffect(() => {
        ...
        return () => {
            cancelled = true;
            if (url)
                URL.revokeObjectURL(url); // ← revoke on replace/remove/unmount
        };
    }, [item]);
    ...
    /* ─────────────── FILLED SLOT ─────────────── */
    return (_jsxs("div", { onDrop: onDrop, onDragOver: onDragOver, onDragLeave: () => setDragOver(false), onMouseEnter: () => setHovered(true), onMouseLeave: () => setHovered(false), className: `group relative h-full w-full overflow-hidden rounded-xl bg-black ring-1 transition ${dragOver ? 'ring-2 ring-primary/80' : 'ring-content/10'}`, children: [isImage ? (_jsx("div", { className: "h-full w-full overflow-hidden", onWheel: onWheel, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: endPan, onPointerLeave: endPan, style: { cursor: zoomable ? (isPanning ? 'grabbing' : 'grab') : 'default', touchAction: 'none' }, children: src ? (_jsx("img", { src: src, alt: item.title, draggable: false, className: "h-full w-full select-none object-contain", style: {
                        transform: `translate(${offset.x}px, ${offset.y}px) scale(${scale})`,
                        transition: isPanning ? 'none' : 'transform 0.15s ease-out',
                    } })) : (_jsx(Loader, { icon: _jsx(ImageIcon, { className: "h-8 w-8 animate-pulse text-content/20" }) })) })) : src ? (_jsx("video", { ref: videoRef, src: src, autoPlay: true, muted: muted, loop: true, playsInline: true, preload: "metadata", className: "h-full w-full bg-black object-contain", poster: videoMeta[item.id]?.thumbnailUrl, onClick: togglePlay, onPlay: () => setPlaying(true), onPause: () => setPlaying(false), onTimeUpdate: () => setCurrent(videoRef.current?.currentTime ?? 0), onLoadedMetadata: () => setDuration(videoRef.current?.duration ?? 0) })) : (_jsx(Loader, { icon: _jsx(Film, { className: "h-8 w-8 animate-pulse text-content/20" }) })), _jsxs("div", { className: `pointer-events-none absolute inset-x-0 top-0 flex items-center gap-2 bg-gradient-to-b from-black/70 to-transparent px-2 py-1.5 transition-opacity ${hovered ? 'opacity-100' : 'opacity-0'}`, children: [_jsx("span", { draggable: true, onDragStart: (e) => {
...
function TileBtn({ onClick, label, disabled, children, }) {
    return (_jsx("button", { onClick: onClick, disabled: disabled, "aria-label": label, title: label, className: "flex h-7 w-7 items-center justify-center rounded-full text-white/90 transition hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-30", children: children }));
}
