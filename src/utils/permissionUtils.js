 ...
 */
export async function verifyPermission(fileHandle, mode = 'read') {
    const h = fileHandle;
    /* Already granted — no prompt, no gesture needed. */
    if (await queryPermission(fileHandle, mode) === 'granted')
        return true;
 ...
 */
export async function verifyPermissions(handles, mode = 'read') {
    const granted = [];
    const denied = [];
 ...
 */
export async function partitionByPermission(handles, mode = 'read') {
    const results = await Promise.all(handles.map(async (h) => [h, await queryPermission(h, mode)]));
    return {
 ...
 */
export async function isDirectoryReadable(handle) {
    try {
        /*
