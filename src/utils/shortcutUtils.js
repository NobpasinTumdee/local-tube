...
export function heldModifiers(e) {
    const out = [];
    if (e.ctrlKey)
        out.push('Control');
    if (e.altKey)
    ...
    if (e.metaKey)
        out.push('Meta');
    return out.sort((a, b) => MODIFIER_ORDER.indexOf(a) - MODIFIER_ORDER.indexOf(b));
}
...
export function validateCombo(combo) {
    if (combo.length < 2)
        return 'Use at least one modifier (Ctrl, Alt, Shift or ⌘) plus a key.';
    const main = combo[combo.length - 1];
    if (isModifierKey(main))
        return 'Finish with a non-modifier key.';
    return null;
}
