import { PASSWORD_MIN_LENGTH, ROOM_ID_MAX_DIGITS, isValidRoomId, } from '../services/p2pProtocol';
/* ─────────────────────────────────────────────────────────────
 *  INVITE LINKS
 * ─────────────────────────────────────────────────────────────
 ...
 * this must say so.
 */
export function generateInviteLink(roomId, password, extra) {
    if (!isValidRoomId(roomId))
        throw new Error('Invalid room ID.');
    if (!password)
        throw new Error('A room password is required to build an invite.');
    const payload = {
        v: INVITE_VERSION,
        r: roomId,
        p: password,
...
function parsePlain(params) {
    return build(params.get('room'), params.get('pwd') ?? params.get('password'));
}
/** Shared validation for both encodings. */
function build(rawRoom, rawPassword) {
    if (typeof rawRoom !== 'string' || typeof rawPassword !== 'string')
        return null;
    const roomId = rawRoom.trim();
    const password = rawPassword;
    if (!isValidRoomId(roomId))
        return null;
    if (roomId.length > ROOM_ID_MAX_DIGITS)
        return null;
    ...
    if (password.length < PASSWORD_MIN_LENGTH || password.length > MAX_PASSWORD_CHARS)
        return null;
