...
/* ─────────────────────────────────────────────────────────────
 *  SCRUB PREVIEW FRAME EXTRACTION
 * ─────────────────────────────────────────────────────────────
 *  Builds a filmstrip for hover-scrubbing by seeking an off-screen <video>
 *  and painting each frame to a canvas.
 *
 *  ── EVERYTHING STAYS LOCAL ──
 *  The source is a blob: URL made from the user's own FileSystemFileHandle,
 *  decoded by the browser, drawn to a canvas in this tab. No fetch, no
 *  upload, and the original file is only ever opened for reading.
 *
 *  ── WHY IT IS SLOW AND WHY THAT IS FINE ──
 *  Each frame costs a seek plus a decode, so a 10-frame strip is ~10 seeks.
 *  That competes with the video the user is actually watching for decoder
 *  bandwidth, which is why extraction is (a) deferred until the player is
 *  settled, (b) serialised through a queue of one, and (c) abortable the
 *  moment the user switches videos.
 *
 *  ── STORAGE ──
 *  Frames are small JPEGs (160px wide, quality 0.55 → roughly 3-5 KB each),
 *  cached in IndexedDB keyed by media id and stamped with the file's
 *  size+mtime so an edited file re-extracts instead of showing stale
 *  frames. An LRU index caps the cache, because a large library would
...
export interface FrameStrip {
  /** `${size}:${lastModified}` — changes when the file changes. */
  signature: string;
  duration: number;
  frames: ScrubFrame[];
}
...
export function signatureFor(file: { size: number; lastModified: number }): string {
  return `${file.size}:${file.lastModified}`;
}

/* ── cache ───────────────────────────────────────────────── */

async function touchIndex(mediaId: string): Promise<void> {
  try {
    const index = (await idbGet<string[]>(INDEX_KEY)) ?? [];
    ...
    for (const id of evicted) await idbDel(CACHE_PREFIX + id).catch(() => undefined);
  } catch {
    /* Cache bookkeeping is best-effort. */
  }
}

export async function readCachedStrip(mediaId: string, signature: string): Promise<FrameStrip | null> {
  try {
    const hit = await idbGet<FrameStrip>(CACHE_PREFIX + mediaId);
    if (!hit || hit.signature !== signature) return null;
...
export async function writeCachedStrip(mediaId: string, strip: FrameStrip): Promise<void> {
  try {
...
function once(el: HTMLVideoElement, type: string, signal?: AbortSignal): Promise<void> {
...
export interface ExtractOptions {
  signal?: AbortSignal;
  /** Called after each frame so the UI can fill in progressively. */
  onFrame?: (frame: ScrubFrame, index: number, total: number) => void;
}
...
export function frameAt(frames: readonly ScrubFrame[], time: number): ScrubFrame | null {
