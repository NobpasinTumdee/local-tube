...
export interface EncryptedPayload {
  /** Schema version, so the format can change without silently mis-reading old data. */
  v: 1;
  salt: Uint8Array;
  iv: Uint8Array;
...
export async function encryptWithKey<T>(data: T, key: CryptoKey, salt: Uint8Array, iterations: number): Promise<EncryptedPayload> {
  /*
...
export async function encryptData<T>(data: T, pin: string): Promise<EncryptedPayload> {
  const salt = randomBytes(SALT_BYTES);
  const key = await deriveKey(pin, salt, PBKDF2_ITERATIONS);
  return encryptWithKey(data, key, salt, PBKDF2_ITERATIONS);
}

export async function decryptWithKey<T>(payload: EncryptedPayload, key: CryptoKey): Promise<T> {
 ...
 */
export async function decryptData<T>(payload: EncryptedPayload, pin: string): Promise<T> {
...
export async function digestMediaId(mediaId: string, digestSalt: Uint8Array): Promise<string> {
  ...
  const hash = await subtle().digest('SHA-256', input.slice(0, digestSalt.length + encoded.length) as BufferSource);
  return bytesToBase64(new Uint8Array(hash));
}

export async function digestMediaIds(ids: readonly string[], digestSalt: Uint8Array): Promise<string[]> {
